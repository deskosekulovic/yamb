...
class Main extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      (prevState.rollCounter !== rollCounter && rollCounter) ||
      (prevState.najavljeno !== najavljeno && rollCounter)
    ) {
  ...
  rollDice() {
    ...
    if (
      numberOfFields - inputCount !== this.najavaAndRucno ||
      !rollCounter ||
      najavljeno
    ) {
      let diceValue = [];
      for (let i = 0; i < numberOfDice; i++) {
        diceValue[i] = selected[i]
          ? dice[i]
  ...
  handleInput(e) {
    ...
    ((column === 'najava' &&
      ((rollCounter === 1 && field === 'kenta' && input) ||
        rollCounter !== 1)) ||
      column !== 'najava') &&
      this.setState({
